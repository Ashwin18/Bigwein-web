...
Route::middleware(['language'])->group(function () {
    // Route::get('/') commented out — frontend homepage handles this
    // Route::get('/', function () {
    ...
    Route::middleware(['auth', 'checkLogin'])->group(function () {
        ...
        // Site Settings (Homepage sections, Filters, Cards, Templates, SEO, Banner)
        Route::get('site-settings',        [\App\Http\Controllers\SiteSettingsController::class, 'index'])->name('site-settings');
        Route::post('site-settings/save',  [\App\Http\Controllers\SiteSettingsController::class, 'save']);

        // Web Translation Manager
        Route::get('web-translations',              [\App\Http\Controllers\WebTranslationController::class, 'index'])->name('web-translations');
        Route::post('web-translations/update',      [\App\Http\Controllers\WebTranslationController::class, 'update']);
        Route::post('web-translations/add-key',     [\App\Http\Controllers\WebTranslationController::class, 'addKey']);
        Route::post('web-translations/delete-key',  [\App\Http\Controllers\WebTranslationController::class, 'deleteKey']);
        Route::resource('language', LanguageController::class);

        // Verify Agent alias
        Route::get('verify_agent', function() {
            return redirect()->to(url('agent-verification'));
        })->name('verify_agent.index');

        // Property Approval alias
        Route::get('property-approval', function() {
            return redirect()->to(url('property?request_status=pending'));
        })->name('property-approval.index');
        Route::get('property-approval/{id}/detail', [PropertController::class, 'show'])->name('property-approval.detail');

        // Reports
        Route::get('reports', [ReportReasonController::class, 'users_reports'])->name('reports.index');
        Route::get('reports-list', [ReportReasonController::class, 'user_reports_list'])->name('reports.list');

        // Amenities
        Route::get('amenities', [OutdoorFacilityController::class, 'index'])->name('amenities.index');
        Route::post('amenities/status', [OutdoorFacilityController::class, 'updateStatus'])->name('amenities.status');

        // Owner Management alias
        Route::get('owner-management', function() {
            return redirect()->to(url('customer'));
        })->name('owner-management.index');


        // Demo Data Settings
        Route::get('demo-settings',             [\App\Http\Controllers\DemoSettingsController::class, 'index']);
        Route::post('demo-settings/toggle',     [\App\Http\Controllers\DemoSettingsController::class, 'toggle']);
        Route::post('demo-settings/seed',       [\App\Http\Controllers\DemoSettingsController::class, 'seed']);
        Route::post('demo-settings/clear',      [\App\Http\Controllers\DemoSettingsController::class, 'clear']);
        ...
        Route::resource('property', PropertController::class)->where(['property' => '[0-9]+']);
        ...
        Route::resource('project', ProjectController::class)->where(['project' => '[0-9]+']);
        /// END :: PROJECT
...
// BIGWEIN FRONTEND ROUTES — loaded last, admin property/project use numeric IDs only
require __DIR__.'/frontend.php';

// BIGWEIN SEARCH V5.5 — register after frontend.php so this implementation wins
// over any older /businesses route without changing the rest of frontend routes.
Route::get('/businesses', [\App\Http\Controllers\Frontend\BusinessController::class, 'index']);
