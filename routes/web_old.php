...
Route::middleware(['language'])->group(function () {
    ...
    Route::middleware(['auth', 'checkLogin'])->group(function () {
        ...
        // Admin Notifications + Charts
        Route::get('bw-admin/notifications',       [\App\Http\Controllers\AdminNotificationsController::class, 'index']);
        Route::post('bw-admin/notifications/read', [\App\Http\Controllers\AdminNotificationsController::class, 'markRead']);
        Route::get('bw-admin/chart-data',          [\App\Http\Controllers\AdminNotificationsController::class, 'chartData']);

        // Owner Management (redirect to customer list)
        Route::get('owner-management', function() { return redirect()->to(url('customer')); });

        // Property Approval
        Route::get('property-approval', function() { return redirect()->to(url('property?request_status=pending')); });
        ...
        Route::post('amenities/status', [OutdoorFacilityController::class, 'updateStatus']);

        // Reports (aliases)
        Route::get('reports', [ReportReasonController::class, 'users_reports'])->name('reports.index');
        Route::get('reports-list', [ReportReasonController::class, 'user_reports_list']);

        // Verify agent alias
        Route::get('verify_agent', function() { return redirect()->to(url('agent-verification')); });
...
// BIGWEIN FRONTEND ROUTES — loaded last so admin routes take priority
