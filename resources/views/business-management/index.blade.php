  ...
  <div class="bw-review-list">
    @forelse($rows as $b)
      <article class="bw-review-row">
        <div class="bw-review-primary"><span class="bw-review-avatar"><i class="bi bi-shop"></i></span><div class="bw-review-primary__copy"><small>Business</small><strong>{{ $b->title }}</strong><span>{{ $b->reference_no ?: 'Listing #'.$b->id }}</span></div></div>
        <div class="bw-review-meta"><small>Business Type · Owner</small><strong>{{ $b->category_name ?: 'Uncategorised' }} · {{ $b->owner_name ?: '—' }}</strong><span class="bw-review-eligibility {{ $b->owner_kyc_status==='approved'?'is-eligible':'is-blocked' }}"><i class="bi {{ $b->owner_kyc_status==='approved'?'bi-unlock':'bi-lock' }}"></i> KYC {{ $b->owner_kyc_status==='approved'?'Approved':'Pending' }}</span></div>
        <div class="bw-review-meta"><small>City · Asking Price</small><strong>{{ $b->city ?: '—' }} · ₹{{ number_format($b->asking_price ?? 0) }}</strong></div>
        <div class="bw-review-meta"><small>Submitted</small><strong>{{ $b->created_at ? \Carbon\Carbon::parse($b->created_at)->format('d M Y') : '—' }}</strong>@include('components.admin.status-badge',['status'=>$b->request_status])</div>
        <div class="bw-review-action">@if($b->request_status!=='draft')<a class="btn {{ $b->request_status==='pending'?'btn-danger':'btn-outline-secondary' }}" href="{{ url('/business-approvals/'.$b->id) }}"><i class="bi bi-eye"></i> {{ $b->request_status==='pending'?'Review':'View Details' }}</a>@else<span class="text-muted small">Seller draft</span>@endif</div>
      </article>
    @empty
