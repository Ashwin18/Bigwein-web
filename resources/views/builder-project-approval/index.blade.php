  ...
  <div class="bw-review-list">
    @forelse($rows as $p)
      <article class="bw-approval-card">
        <div class="bw-approval-card__left"><div class="bw-review-primary"><span class="bw-review-avatar"><i class="bi bi-buildings"></i></span><div class="bw-review-primary__copy"><small>Project</small><strong>{{ $p->title }}</strong><span>{{ $p->company_name ?: $p->owner_name }}</span></div></div><div class="bw-approval-card__metadata"><span><i class="bi bi-tag"></i>{{ $p->project_segment ? ucwords(str_replace('_',' ',$p->project_segment)) : 'Uncategorised' }}</span><span><i class="bi bi-geo-alt"></i>{{ $p->city ?: '—' }}</span></div></div>
        <div class="bw-approval-card__middle"><div class="bw-approval-fact"><small>KYC Status</small><span class="bw-review-eligibility {{ $p->owner_kyc_status==='approved'?'is-eligible':'is-blocked' }}"><i class="bi {{ $p->owner_kyc_status==='approved'?'bi-unlock':'bi-lock' }}"></i> {{ $p->owner_kyc_status==='approved'?'Approved':'Pending' }}</span></div><div class="bw-approval-fact"><small>Submitted</small><strong>{{ $p->created_at ? \Carbon\Carbon::parse($p->created_at)->format('d M Y') : '—' }}</strong></div><div class="bw-approval-fact"><small>RERA</small><strong>{{ $p->rera_number ?: 'Not provided' }}</strong></div></div>
        <div class="bw-approval-card__right"><div class="bw-approval-card__badges">@include('components.admin.status-badge',['status'=>$p->request_status]) @include('components.admin.status-badge',['status'=>$p->status ? 'active':'inactive'])</div><a class="btn {{ $p->request_status==='pending'?'btn-danger':'btn-outline-secondary' }}" href="{{ url('/builder-project-approvals/'.$p->id) }}"><i class="bi bi-eye"></i> {{ $p->request_status==='pending'?'Review':'View' }}</a></div>
      </article>
    @empty
