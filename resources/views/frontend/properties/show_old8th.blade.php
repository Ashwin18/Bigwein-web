...
@php
  ...
  $city     = implode(', ', array_filter([$prop->city, $prop->state, $prop->country]));
  $mainImg  = $prop->title_image ?: '';  // accessor returns full URL
  $gallery  = $prop->gallery ?? collect();  // Collection of PropertyImages with image_url
...
<div class="container" style="padding-top:24px;padding-bottom:60px;">
  ...
  <div class="detail-gallery">
    ...
    @if($gallery->count() >= 1)
      <div class="gallery-sub">
        <img src="{{ $gallery[0]->image_url }}" alt="Gallery" style="width:100%;height:100%;object-fit:cover;cursor:pointer;" onclick="setMain('{{ $gallery[0]->image_url }}')"/>
      </div>
    @endif
    @if($gallery->count() >= 2)
      <div class="gallery-sub" style="position:relative;">
        <img src="{{ $gallery[1]->image_url }}" alt="Gallery" style="width:100%;height:100%;object-fit:cover;cursor:pointer;" onclick="setMain('{{ $gallery[1]->image_url }}')"/>
        @if($gallery->count() > 2)
  ...
  @if($gallery->count())
    <div class="thumb-strip">
      ...
      @foreach($gallery->take(8) as $gi)
        <img src="{{ $gi->image_url }}" onclick="setMain('{{ $gi->image_url }}')" style="cursor:pointer;" onerror="this.style.display='none'"/>
      @endforeach
    </div>
  @endif
  ...
  <div class="detail-layout">
    ...
    <div>
      ...
      <div class="detail-params">
        @if($pBeds)<div class="detail-param"><i class="fa-solid fa-bed"></i><strong>{{ $pBeds['value'] ?? '-' }}</strong><span>Bedrooms</span></div>@endif
        @if($pBaths)<div class="detail-param"><i class="fa-solid fa-bath"></i><strong>{{ $pBaths['value'] ?? '-' }}</strong><span>Bathrooms</span></div>@endif
        @if($pArea)<div class="detail-param"><i class="fa-solid fa-ruler-combined"></i><strong>{{ $pArea['value'] ?? '-' }}</strong><span>sq.ft</span></div>@endif
        @if($pFloor)<div class="detail-param"><i class="fa-solid fa-building-columns"></i><strong>{{ $pFloor['value'] ?? '-' }}</strong><span>Floor</span></div>@endif
      ...
      <div class="detail-box">
        ...
        <div class="amenities-list">
          @foreach($params as $param)
            <span class="amenity">
              <i class="fa-solid fa-check" style="color:var(--red);"></i>
              {{ $param['translated_name'] ?? $param['name'] ?? '' }}
              @if(!empty($param['value'])): {{ $param['value'] }}@endif
            </span>
          @endforeach
        </div>
        <div class="feature-table" style="margin-top:16px;">
          <div class="feat-row"><span class="feat-label">Type</span><span class="feat-val">{{ $prop->category?->category ?? '—' }}</span></div>
          <div class="feat-row"><span class="feat-label">For</span><span class="feat-val">{{ $pType==0?'Sale':'Rent' }}</span></div>
          <div class="feat-row"><span class="feat-label">City</span><span class="feat-val">{{ $prop->city ?? '—' }}</span></div>
          <div class="feat-row"><span class="feat-label">State</span><span class="feat-val">{{ $prop->state ?? '—' }}</span></div>
          <div class="feat-row"><span class="feat-label">Address</span><span class="feat-val">{{ \Illuminate\Support\Str::limit($prop->address ?? '—',50) }}</span></div>
          <div class="feat-row"><span class="feat-label">Listed</span><span class="feat-val">{{ $prop->created_at?->diffForHumans() ?? '—' }}</span></div>
          <div class="feat-row"><span class="feat-label">Views</span><span class="feat-val">{{ $prop->total_click ?? 0 }}</span></div>
        </div>
      </div>
      @endif
    ...
    <div>
      <div class="contact-card">
        ...
        @endif
        <button class="contact-btn btn-msg" onclick="openEnquiry()"><i class="fa-solid fa-envelope"></i> Send Message</button>
  ...
  <div style="margin-top:48px;padding-top:40px;border-top:1px solid var(--border);">
    <div class="sec-row"><div><h2 class="sec-title">Similar Properties</h2><p class="sec-sub">You may also like</p></div><a href="/properties" class="btn-all">View All <i class="fa-solid fa-arrow-right"></i></a></div>
...
<div class="enquiry-modal" id="enqModal" onclick="if(event.target===this)closeEnq()">
  <div class="enquiry-box">
    ...
    <div class="form-field"><label class="form-label">Your Name</label><input class="form-input" id="enqName" type="text" placeholder="Full name"/></div>
    ...
    <div class="form-field" style="margin-top:14px;"><label class="form-label">Message</label><textarea class="form-input" id="enqMsg" rows="3" style="resize:vertical;">I am interested in this property. Please contact me.</textarea></div>
    <button class="btn-auth" id="enqBtn" onclick="submitEnq()" style="margin-top:16px;"><i class="fa-solid fa-paper-plane"></i> Send Message</button>
  </div>
</div>
...
async function handleSave(){
  if(!BW.isLoggedIn){ bwToast('Please login to save properties','error'); setTimeout(()=>window.location.href='/login',900); return; }
...
function openEnquiry(){
  if(!BW.isLoggedIn){ bwToast('Please login to contact the owner','error'); setTimeout(()=>window.location.href='/login',900); return; }
  const u = BW.customer;
