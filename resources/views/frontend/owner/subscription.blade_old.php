...
<div class="plans-grid">
  ...
  <div class="plan-card {{ $plan[4] }} {{ $i===2?'popular':'' }} {{ (!$activePlan && $i===0)?'current':'' }}">
    ...
    <button class="plan-btn current-btn"><i class="fa-solid fa-circle-check"></i> Current Plan</button>
    @else
    <button class="plan-btn" @if($plan[2]==='0') disabled @else onclick="showPaymentNote('{{ $plan[0] }}')" @endif>
...
async function subscribePlan(pkgId, name, price) {
    if (!confirm(`Subscribe to ${name} plan for ₹${price.toLocaleString('en-IN')}?`)) return;
...
function showPaymentNote(name) {
    showToast(`Payment gateway integration needed for ${name} plan. Contact admin.`, 'success');
}
</script>
@endpush
