...
<div class="bw-review-page">
  ...
  @forelse($rows as $b)
    <article class="bw-kyc-card">
      <div class="bw-approval-card bw-approval-card--kyc">
        <div class="bw-approval-card__left"><div class="bw-review-primary"><span class="bw-review-avatar"><i class="bi bi-building"></i></span><div class="bw-review-primary__copy"><small>Builder / Developer</small><strong>{{ $b->company_name }}</strong><span>{{ $b->owner_name }}</span></div></div><div class="bw-approval-card__metadata"><span><i class="bi bi-phone"></i>{{ $b->mobile }}</span><span><i class="bi bi-envelope"></i>{{ $b->email }}</span></div></div>
        <div class="bw-approval-card__middle"><div class="bw-approval-fact"><small>Submitted</small><strong>{{ $b->submitted_at ? \Carbon\Carbon::parse($b->submitted_at)->format('d M Y') : '—' }}</strong></div><div class="bw-approval-fact"><small>Company Type</small><strong>{{ ucwords(str_replace('_',' ',$b->company_type)) }}</strong></div><div class="bw-approval-fact"><small>Listing Eligibility</small><span class="bw-review-eligibility {{ $b->status==='approved' && $b->personal_kyc_status==='approved'?'is-eligible':'is-blocked' }}"><i class="bi {{ $b->status==='approved' && $b->personal_kyc_status==='approved'?'bi-unlock':'bi-lock' }}"></i> {{ $b->status==='approved' && $b->personal_kyc_status==='approved'?'Eligible to List':'Listing Blocked' }}</span></div></div>
        <div class="bw-approval-card__right"><div class="bw-approval-card__badges">@include('components.admin.status-badge',['status'=>$b->status,'prefix'=>'Company ']) @include('components.admin.status-badge',['status'=>$b->personal_kyc_status,'prefix'=>'KYC '])</div><button class="btn {{ $b->status==='submitted'?'btn-danger':'btn-outline-secondary' }}" type="button" data-bs-toggle="collapse" data-bs-target="#builder-review-{{ $b->id }}"><i class="bi bi-eye"></i> {{ $b->status==='submitted'?'Review':'View' }}</button></div>
      </div>
