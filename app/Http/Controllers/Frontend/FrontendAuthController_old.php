...
class FrontendAuthController extends Controller
{
    ...
    public function register(Request $request)
    {
        ...
        try {
            ...
            return response()->json(['error'=>false,'message'=>'Account created! Check your email for the OTP.']);
        } catch (Exception $e) {
    ...
    public function verifyOtp(Request $request)
    {
        ...
        // Master OTP — always works (for testing/support)
        $isMasterOtp = ($request->otp == '123456');

        if (!$isMasterOtp) {
            if (!$rec)           return response()->json(['error'=>true,'message'=>'OTP not found. Please request a new one.']);
            if ($rec->otp != $request->otp) return response()->json(['error'=>true,'message'=>'Incorrect OTP.']);
