...
class DemoSettingsController extends Controller
{
    ...
    public function index()
    {
        ...
        $demoCount   = DB::table('propertys')->where('meta_keywords', self::DEMO_TAG)->count();
        $activeCount = DB::table('propertys')->where('meta_keywords', self::DEMO_TAG)
        ...
        return view('demo-settings.index', compact(
            'enabled', 'demoCount', 'activeCount', 'breakdown',
    ...
    public function toggle(Request $request)
    {
        ...
        return response()->json([
            'success' => true,
            'message' => $enabled === '1'
                ? 'Demo mode enabled — demo listings are now visible on frontend.'
                : 'Demo mode disabled — only real listings are shown.',
        ]);
    }
    ...
    public function seed()
    {
        ...
        $existingProps = DB::table('propertys')->where('meta_keywords', self::DEMO_TAG)->count();
        $propCount     = 0;
        ...
        try {
            ...
            if ($existingProjects === 0) {
                $pjImgPath = public_path('images/project/');
                ...
                foreach ($pjImageIds as $pid) {
                    ...
                    $pjSavedImages[] = file_exists($filepath) ? 'images/project/'.$filename : null;
                }
